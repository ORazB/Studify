...
<body>
    <section class="create bg-gradient-to-br from-teal-100 to-gray-100 flex items-center justify-center min-h-screen p-4">
        <div
            ...
            <!-- Heading -->
            <h1 class="text-3xl font-bold text-gray-800 mb-3 text-center">Tambah SPP Baru</h1>
            <div class="line w-20 h-1 bg-teal-500 mx-auto mb-6 rounded-full"></div>

            <!-- Form -->
            <form action="{{ route('spp.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <input type="number" id="nominal" name="nominal" required min="0" step="1000"
                        placeholder="Contoh: 150000" value="{{ old('nominal') }}"
                        class="w-full border border-gray-300 rounded-lg px-4 py-3 text-gray-700
                               focus:outline-none focus:ring-2 focus:ring-teal-400 focus:border-transparent transition duration-200">
                    @error('nominal')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
